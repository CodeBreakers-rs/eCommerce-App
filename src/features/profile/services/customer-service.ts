import type { CustomerUpdatePayload, CustomerProfile, BasicProfile } from '../../../types/customer';
...
export const getCustomerProfile = async (token: string,): Promise<CustomerProfile | null> => {
  try {
...
console.log('Using token:', accessToken)
  ...
  if (!response.ok) {
    ...
    throw new Error(error.message || JSON.stringify(error) || 'Failed to update customer profile')
  }
